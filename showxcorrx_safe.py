...
import numpy as np
import scipy as sp
from numpy import argmax, pi, r_, zeros
...
def _get_null_distribution(
    ...
    """
    print("estimating significance distribution using {0} " "repetitions".format(numreps))
    ...
    for i in range(numreps):
        ...
        theshuffledxcorr = tide_corr.fastcorrelate(
            filteredindata, filteredshuffleddata, usefft=dofftcorr, weighting=corrweighting
        )
...
def _get_parser():
    ...
    parser.add_argument(
        "Fs", type=lambda x: is_float(parser, x), help="The sample rate of the timecourses, in Hz"
    )
    ...
    parser.add_argument(
        "-T", dest="trimdata", action="store_true", help=("Trim data to match"), default=False
    )
    parser.add_argument(
        "-A",
        dest="summarymode",
    ...
    parser.add_argument(
        ...
        metavar="TRIALS",
        type=int,
        help=("Estimate significance thresholds by Monte " "Carlo with TRIALS repetition"),
        default=0,
    )
    ...
    parser.add_argument(
        "--verbose", dest="verbose", action="store_true", help=("Print things"), default=False
    )
    return parser
...
def showxcorrx_workflow(
    ...
    thexcorr = tide_corr.fastcorrelate(
        filtereddata1, filtereddata2, usefft=dofftcorr, weighting=corrweighting, displayplots=debug
    )
    ...
    if calccepstraldelay:
        ...
        print("cepstral delay time is {0}, correlation is {1}".format(cepdelay, cepcoff))
    ...
    sampletime = 1.0 / Fs
    xcorr_x = r_[0:xcorrlen] * sampletime - (xcorrlen * sampletime) / 2.0 + sampletime / 2.0
    ...
    if verbose:
        if uselabel:
            ...
            print(infilename1, "[0 seconds] == ", infilename2, "[", -1 * maxdelay, " seconds]")
    ...
    if summarymode:
        if estimate_significance:
            if uselabel:
                ...
                print(thelabel, thepxcorr[0], pearpcts_fit[0], R, pcts_fit[0], -1 * maxdelay)
            else:
                if labelline:
                    print(
                        "pearson_R",
...
def _main(argv=None):
    ...
    if args["arbvec"] is not None:
        if len(args["arbvec"]) == 2:
            ...
            raise ValueError("Argument '-F' or '--arbvec' must be either two " "or four floats.")
