...
+++++++++++++++++++++++++
When we started this whole research effort, we were originally planning to denoise NIRS data, not fMRI data.  But one thing led to another, and the NIRS got derailed for the fMRI effort.  Now that we have some time to catch our breaths, and more importantly, we have access to some much higher quality NIRS data, this moved back to the front burner.  The majority of the work was already done, we just needed to account for a few qualities that make NIRS data different from fMRI data:
