...
class proberegressor:
    ...
    def maketargettimeaxis(self):
        ...
        os_fmri_x = (
            np.arange(
                0.0,
                (validtimepoints - optiondict["addedskip"]) * self.targetoversample
                - (self.targetoversample - 1),
            )
            * self.targetoversample
            * self.targetperiod
            + skiptime
        )
...
class correlator:
    ...
    def setreftc(self, reftc):
        ...
        # make the time axis
        self.timeaxis = (
            np.arange(0.0, self.corrlen) * (1.0 / self.Fs)
    ...
    def trim(self, vector):
        return vector[
            self.corrorigin - self.lagmininpts : self.corrorigin + self.lagmaxinpts
        ]
    ...
    def run(self, thetc, trim=True):
        ...
        self.thexcorr = tide_corr.fastcorrelate(
...
class correlation_fitter:
    ...
    def _maxindex_noedge(self, corrfunc):
        ...
        while not done:
            ...
            if self.bipolar:
                minindex = (
                    np.argmax(np.fabs(corrfunc[lowerlim:upperlim])) + lowerlim
                ).astype("int32")
