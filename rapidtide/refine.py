...
def refineregressor(
    ...
    if tide_stats.getmasksize(refinemask) == 0:
        if (includemask is None) and (excludemask is None):
            print(
                "ERROR: no voxels in the refine mask - relax ampthresh, delaythresh, or sigmathresh - exiting"
            )
            sys.exit()
        else:
            print(
                "ERROR: no voxels in the refine mask - change include/exclude masks or relax ampthresh, delaythresh, or sigmathresh - exiting"
            )
            sys.exit()
