...
"""def _procOneVoxelFitcorr(vox, corrtc, corrscale, genlagtc, initial_fmri_x, optiondict, displayplots, initiallag=None,
                         rt_floatset=np.float64,
                         rt_floattype='float64'
                         ):
...
"""def fitcorr(genlagtc, initial_fmri_x, lagtc, slicesize,
    ...
    print('*************entering the spiffy new factored fitcorr!*************')
    displayplots = False
    ...
    if optiondict['nprocs'] > 1:
        ...
        data_out = tide_multiproc.run_multiproc(fitcorr_consumer, inputshape, None,
        ...
        for vox in range(0, inputshape[0]):
            ...
            if initiallags is None:
                ...
                if initiallags[vox] != 0.0:
                    ...
                    R2[vox], lagmask[vox], failreason = \
                        _procOneVoxelFitcorr(vox, corrout[vox, :], corrscale, genlagtc, initial_fmri_x, optiondict,
                                             displayplots, initiallag=initiallags[vox],
                                             rt_floatset=rt_floatset,
                                             rt_floattype=rt_floattype
                                             )
                    volumetotal += volumetotalinc
...
#
#
"""def onecorrfit_old(thetc, corrscale, optiondict, disablethresholds=False, displayplots=False, initiallag=None,
                   rt_floatset=np.float64,
                   rt_floattype='float64'
                   ):
    if initiallag is not None:
        ...
        def fitcorr_consumer(inQ, outQ):
            while True:
                try:
                    ...
                    outQ.put(_procOneVoxelFitcorr(val,
                                                     corrout[val, :],
                                                     corrscale,
                                                     genlagtc,
                                                     initial_fmri_x,
