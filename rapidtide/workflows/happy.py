...
def happy_main(thearguments):
    ...
    for o, a in opts:
        ...
        if o == "-x":
            ...
            newval = float(a)
            print(
                "Will set bottom of cardiac search range to",
                newval,
                "BPM from",
                minhr,
                "BPM",
            ...
            newval = float(a)
            print(
                "Will set top of cardiac search range to",
                newval,
                "BPM from",
                maxhr,
                "BPM",
    ...
    if motionfilename is not None:
        ...
        motionregressors, filtereddata = tide_glmpass.motionregress(
            motionfilename,
    ...
    for thispass in range(numpasses):
        ...
        if thispass == numpasses - 1:
            ...
            if saveintermediate:
                tide_io.writevec(
                    cycleaverage,
                    outputroot + "_cycleaverage_pass" + str(thispass + 1) + ".txt",
                )
                tide_io.writevec(
                    cardfromfmri_sliceres,
                    outputroot
        ...
        if thispass == numpasses - 1:
            ...
            tide_io.writevec(
                cardfromfmrienv_stdres,
                outputroot + "_cardfromfmrienv_" + str(stdfreq) + "Hz.txt",
            )
        else:
            if saveintermediate:
                tide_io.writevec(
        ...
        if dodlfilter:
            if dlfilterexists:
                ...
                stdtimeaxis = (1.0 / stdfreq) * sp.linspace(
        ...
        if outputlevel > 0:
            if thispass == numpasses - 1:
                ...
                tide_io.writevec(
                    instantaneous_phase, outputroot + "_instphase_unwrapped.txt"
                )
        ...
        upsampledslicetimeaxis = sp.linspace(
            0.0,
            tr * timepoints,
            num=(timepoints * numsteps * upsamplefac),
            endpoint=False,
        )
        ...
        if doaliasedcorrelation and (thispass == numpasses - 1):
            ...
            thecorrelator = tide_corr.aliasedcorrelator(
                ...
                mrsamplerate,
                corrsearchvals,
                padvalue=aliasedcorrelationwidth,
            )
    ...
    if doglm:
        ...
        if spatialglmdenoise:
            ...
            timings.append(
                [
                    "Cardiac signal regression finished",
                    time.time(),
                    timepoints,
                    "timepoints",
                ]
        ...
        timings.append(
            ["Cardiac signal regression files written", time.time(), None, None]
        )
