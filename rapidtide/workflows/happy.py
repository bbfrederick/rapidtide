...
def _procOneVoxelNormalize(vox, voxels, detrendorder=0):
    if detrendorder > 0:
        normtc = tide_math.pcnormalize(tide_fit.detrend(voxels[vox, :], detrendorder=detrendorder, demean=False))
    else:
        normtc = tide_math.pcnormalize(voxels[vox, :])
    return vox, normtc


def tcnorm(validimbyvox, nprocs=1, detrendorder=0):
    """
    ...
    if nprocs > 1:
        ...
        def normalize_consumer(inQ, outQ):
            while True:
                try:
                    ...
                    outQ.put(_procOneVoxelNormalize(val, validimbyvox, detrendorder=detrendorder))
        ...
        for vox in range(0, inputshape[0]):
            ...
            dummy, normvoxels[vox, :] = _procOneVoxelNormalize(vox, validimbyvox, detrendorder=detrendorder)
            volumetotal += 1
