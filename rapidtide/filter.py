    ...
    if lowerpass <= 0.0:
        ...
        if usebutterworth:
            ...
            if usetrapfftfilt:
                ...
                return dolpfftfilt(
                    Fs, upperpass, inputdata, padlen=padlen, cyclic=False, debug=debug
                ).astype(np.float64)
        ...
        if usebutterworth:
            ...
            if usetrapfftfilt:
                ...
                return dohpfftfilt(
                    Fs, lowerpass, inputdata, padlen=padlen, cyclic=False, debug=debug
                )
    else:
        # set up for bandpass
        if usebutterworth:
...
class plethfilter:
    def __init_(self, Fs, Fl, Fh, order=4, attenuation=20):
        ...
        self.order = order
        self.b, self.a = scipy.signal.cheby2(
            self.order,
            self.attenuation,
