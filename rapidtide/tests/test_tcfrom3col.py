...
from rapidtide.workflows.tcfrom3col import _get_parser, tcfrom3col


def _make_default_args(**overrides):
    defaults = dict(
        infilename="input_3col.txt",
        timestep=0.5,
        numpoints=6,
...
def tcfrom3col_basic(debug=False):
    if debug:
        print("tcfrom3col_basic")
    ...
    def _mock_maketcfrom3col(indata, timeaxis, outdata, debug=False):
    ...
    with (
        patch("rapidtide.workflows.tcfrom3col.tide_io.readvecs", return_value=inputdata),
        patch(
            "rapidtide.workflows.tcfrom3col.tide_util.maketcfrom3col",
            side_effect=_mock_maketcfrom3col,
        ),
        patch("rapidtide.workflows.tcfrom3col.tide_io.writenpvecs", side_effect=_mock_writenpvecs),
    ):
        tcfrom3col(args)
...
def tcfrom3col_debug_propagates(debug=False):
    if debug:
        print("tcfrom3col_debug_propagates")
    args = _make_default_args(debug=True)
    inputdata = np.array([[0.0, 1.0, 10.0]])
    captured = {}
    ...
    with (
        ...
        patch("rapidtide.workflows.tcfrom3col.tide_io.writenpvecs"),
    ):
        tcfrom3col(args)
...
def test_tcfrom3col(debug=False):
    parser_basic(debug=debug)
    parser_required_args(debug=debug)
    parser_defaults(debug=debug)
    parser_debug_flag(debug=debug)

    tcfrom3col_basic(debug=debug)
    tcfrom3col_debug_propagates(debug=debug)


if __name__ == "__main__":
    test_tcfrom3col(debug=True)
