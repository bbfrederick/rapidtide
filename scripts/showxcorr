...
#
# $Author: frederic $
#       $Date: 2016/06/14 13:50:53 $
#       $Id: showxcorr,v 1.39 2016/06/14 13:50:53 frederic Exp $
#
...
def usage():
    ...
    print("    -g 	     - perform phase alignment transform (phat) rather than ")
...
try:
    opts, args = getopt.getopt(sys.argv[4:], "fN:r:z:aATtVLRCF:dl:s:D:wg", ["help"])
...
if gccphat:
    thexcorr = tide.gccphat(filtereddata1, filtereddata2, doplot)
else:
    thexcorr = tide.fastcorrelate(filtereddata1, filtereddata2, usefft=dofftcorr)
...
if dofindmaxlag:
    ...
    maxindex, maxdelay, maxval, maxsigma, maskval, failreason = tide.findmaxlag(
            xcorr_x_trim,
            thexcorr_trim,
            -searchrange,searchrange,1000.0,
            refine=True,
            useguess=False,
            fastgauss=False,
            displayplots=False)
...
if showpearson and verbose:
    ...
    if estimate_significance:
        for idx, percentile in enumerate(thepercentiles):
            print("    pear_p("+str(1.0 - percentile)+"):\t",pearpcts[idx])
    print("")
...
if verbose:
    if (uselabel == True):
        ...
        if estimate_significance:
            for idx, percentile in enumerate(thepercentiles):
                print("    xc_p("+str(1.0 - percentile)+"):\t",pcts[idx])
